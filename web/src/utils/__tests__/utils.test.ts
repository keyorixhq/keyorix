...
describe('generateId', () => {
    ...
    it('falls back to a getRandomValues UUIDv4 when randomUUID is undefined (insecure HTTP context)', () => {
        const original = crypto.randomUUID;
        try {
            // Simulate plain-HTTP-on-a-LAN-IP: crypto.randomUUID is undefined there.
            (crypto as unknown as { randomUUID?: unknown }).randomUUID = undefined;
            const id = generateId();
            // RFC4122 v4 shape (8-4-4-4-12, version nibble 4, variant 8/9/a/b).
