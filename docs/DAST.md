...
|-------|---------|--------|
| #1213, #1273 (closed) | `ZAP/10055: CSP: style-src unsafe-inline` | Required by the SPA's CSS-in-JS / Tailwind runtime. `script-src` does not carry `unsafe-inline`, so inline script injection (the primary XSS vector) remains blocked. This is a genuine (accepted) CSP gap, not a scanner false positive, so it isn't suppressed and is expected to keep recurring on every full scan — close each occurrence with this rationale. |
