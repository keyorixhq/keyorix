...
|---|---|---|---|---|
| `notifications.project_id` | `NULL` | System/operational notice not tied to one project (e.g. "backup completed") | **Decided (2026-09-07): platform-scoped, not blanket-visible.** `project_id = current_tenant OR (project_id IS NULL AND current_setting('app.is_platform_admin', true) = 'true')` — reversed from this row's original draft (`OR project_id IS NULL`, visible to every tenant). Tenants see only their own project's notifications; a NULL/global notice is visible only to a session with the platform-admin flag set. Rationale: operational notices are operator-facing, not tenant-facing — a tenant gains nothing from seeing platform-wide operational timing, and an attacker gains reconnaissance value from it, which is the PR #1774 shape (global rows visible to everyone) rebuilt inside the policy layer. If a future feature needs a genuinely tenant-visible global announcement, it fans out explicitly at write time — one row per tenant — rather than relying on a NULL sentinel to mean "everybody." | Decided, not blocking. |
...
| `audit_events.project_id` | `NULL` | Platform/global-scope event (admin login, global RBAC change) | **Decided (2026-09-07): platform-scoped, not blanket-visible.** `project_id = current_tenant OR (project_id IS NULL AND current_setting('app.is_platform_admin', true) = 'true')`. Tenants see their own project's audit events only; a NULL/platform-scope event (admin login, global RBAC change) is visible only to a session with the platform-admin flag set — never blanket-unioned via a bare `OR project_id IS NULL`. Rationale: global admin-login and RBAC-change events reveal platform operational timing to every tenant with no value to them and real intelligence value to an attacker, and "global rows visible to all" is the PR #1774 exposure shape rebuilt inside the RLS policy layer itself — exactly the failure this ADR exists to prevent, just moved one layer down. | Decided, not blocking. |
...
row through the request path). `notifications` and `audit_events` get the
same write-side treatment now that their read-side rule is decided above:
`WITH CHECK (project_id = current_tenant)` only — the runtime role can never
write a NULL/platform-scope row through the request path for either table,
matching the read side's own platform-admin gate. A platform-scope
notification or audit event is written exclusively via `keyorix_migrator` or
a dedicated admin path, never through the ordinary request-serving role's
`WITH CHECK`.

**Standing rule, now that this has recurred three times in this ADR alone:
a `NULL`/sentinel column must never be given "visible to all" semantics in
this schema again.** `notifications` and `audit_events` both started this
section with a blanket `OR project_id IS NULL` as their first-drafted read
rule, and both were corrected to platform-admin-gated instead of tenant-wide
once traced back to the PR #1774 shape it would have rebuilt. Combined with
the pre-existing blank-`AccountState` incident (#1723, cited at the top of
this section) that required a dedicated backfill and `CHECK` constraint after
the fact, this is the third time an ambiguous/absent value has needed a
security-relevant decision walked back from "everyone can see this" to
something scoped — the same defect class each time, not three unrelated
incidents. Any future table added to this ADR's scope with a nullable
tenancy column defaults to the platform-admin-gated shape (`project_id =
current_tenant OR (project_id IS NULL AND
current_setting('app.is_platform_admin', true) = 'true')`), not a blanket
`OR project_id IS NULL`, unless a specific table earns an explicit,
written exception the way `rotation_policies` did above.
...
- The two §1b decisions that were open (`audit_events` NULL-scope visibility,
  `notifications` global-scope visibility) are **resolved (2026-09-07)**: both
  are platform-scoped, gated on an explicit `app.is_platform_admin` session
  flag, never blanket-visible to every tenant. ADR-103 is unblocked for
  implementation on this axis — see §1b for the decided policies and the
  standing "no NULL-means-everyone" rule they establish. What remains before
  an implementation PR can start is unchanged from the rest of this document:
  the §1f verification design (four tests plus the context-propagation guard,
  red-then-green) must exist and pass, and the required latency measurement
  below must run.
