...
- **Single-use.** Consuming a token transitions it `active → consumed` (`ConsumeSetupToken`, one atomic UPDATE) *before* the separate step that materializes its effect (creates the session / accepts the invitation / sets the password). **Corrected 2026-09-07**: these were previously described as the same transaction; they are two separate operations. This is deliberate, not an oversight — the token is burned up front so a replay can never re-attempt consumption, even if materialization fails afterward; the failure mode is a consumed-but-not-materialized token (fails safe: no double-consume, no effect applied twice), not a security gap. A replay finds `consumed` and is rejected either way.
