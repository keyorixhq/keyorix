...
if command -v curl >/dev/null 2>&1; then
    curl --proto "$HTTPS_PROTO" --tlsv1.2 -fsSL "${CHECKSUMS_BASE}/checksums.txt" -o "$TMP_CHECKSUMS" || error "Failed to download checksums.txt" # NOSONAR -- bash:S6506 false positive: --proto "$HTTPS_PROTO" --tlsv1.2 enforces HTTPS; redirect-following is required for GitHub release CDN
else
...
if command -v cosign >/dev/null 2>&1; then
    if command -v curl >/dev/null 2>&1; then
        curl --proto "$HTTPS_PROTO" --tlsv1.2 -fsSL "${CHECKSUMS_BASE}/checksums.txt.pem" -o "$TMP_CHECKSUMS_PEM" \
