// secret_scan_report.go — printScanReport, sanitizeScanName, isScanPlaceholder.
package cmd
...
func printScanReport(report *ScanReport) {
	...
	if len(hardcoded) > 0 {
		fmt.Printf("Hardcoded in source code (%d) — HIGHEST RISK:\n", len(hardcoded))
	...
	if len(configFiles) > 0 {
		fmt.Printf("Found in config files (%d):\n", len(configFiles))
	...
	fmt.Println("  keyorix secret scan . --import           Show a hint to import all found secrets into Keyorix")
...
func isScanPlaceholder(s string) bool {
