...
// method, carried forward from PR 6 into PR 7) before returning -- unlike PR 1/2's
// per-command clients, which only checked skew from `login`/`status`. A Refuse result
// blocks the command outright; a Warning is printed to stderr (so it never pollutes stdout
// output a script might parse) and the command proceeds.
...
// PR 6/PR 7 additions and their doc comments for why a full typed schema wasn't worth it for
...
// forward into PR 7, docs/cli-split-inventory.md §7): "surface ... refusals readably."
