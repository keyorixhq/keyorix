...
//
// Ported from a duplicate PR (#1993, closed as superseded by this one) --
// adapted to use setClock/SetClockForTesting (this PR's seam) instead of
// direct field assignment where that seam already covers the site.
//
...
func TestOIDCVerify_ExpNbfCheckedAgainstInjectedClock_NotRealTime(t *testing.T) {
	...
	// Pin the verifier's clock to `pinned` via this PR's seam. If the
	// library's exp/nbf check still read real time.Now() (the mixed-clock bug
	// #1983 fixes), this would fail identically to the control. Success
	// proves the library's check and the max-age check below both used
	// v.effectiveNow().
...
func TestVerifyIDToken_ExpNbfCheckedAgainstInjectedClock_NotRealTime(t *testing.T) {
	...
	c := &KeyorixCore{now: time.Now, ssoJWKS: staticResolver{kid: "kid-1", key: &key.PublicKey}}
