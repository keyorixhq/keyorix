...
// clients. Traced (see the #1679 follow-up round's Step 1 report): the
// mutation (e.g. CreateSecret) and its audit write are separate SQLite
// transactions with no atomicity between them -- emitAudit
...
// TestConcurrency_AuditPersist_LostOnContention is the red case: proves the
// gap exists today. Many goroutines each create a real secret and then
...
// that DID succeed having no corresponding audit record. Today's code
// (emitAudit-equivalent: log the error, drop the event) violates that.
func TestConcurrency_AuditPersist_LostOnContention(t *testing.T) {
	...
	for i := 0; i < writers; i++ {
		...
		go func(n int) {
			...
			if err != nil {
				...
				// not what this test is about; today's real emitAudit
				// never even gets called in that case either (there is
				// nothing to audit if the mutation never happened).
				return
			}
			...
			// Mirrors internal/core/service.go's emitAudit on failure: log
			// and drop, no retry, no queue -- this IS today's real
			// behavior, reproduced at the storage layer directly rather
			// than through the full core+HTTP-handler+goSafe stack, to
			// isolate the storage-contention question from the separate,
			// already-reported async-dispatch problem.
