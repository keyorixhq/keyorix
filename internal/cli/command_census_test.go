...
var commandCensus = map[string]censusEntry{
	...
	"billing report":                      {censusMoved, "FINISH-SPLIT census-gaps", "ported the REST/remote branch only (cli/cmd/billing.go); local/embedded branch dropped, not carried forward -- Finding S17"},
	...
	"bundle import":                       {censusMoved, "PR 10, #2076 (open)", "verification code moved into pkg/bundleverify (no core/storage/config/SDK imports)"},
	"bundle verify":                       {censusMoved, "PR 10, #2076 (open)", "verification code moved into pkg/bundleverify (no core/storage/config/SDK imports)"},
	...
	"license install":                     {censusMoved, "PR 10, #2076 (open)", "verification code moved into pkg/licenseverify (no core/storage/config/SDK imports)"},
	...
	"license status":                      {censusMoved, "PR 10, #2076 (open)", "verification code moved into pkg/licenseverify (no core/storage/config/SDK imports)"},
	...
	"run":                                 {censusMoved, "PR 10, #2076 (open)", "the local/embedded fetch branch was dropped, not ported -- Finding S18: it had zero authorization check and zero audit event, so dropping it is a security fix, not just cleanup"},
	...
	"system init":                         {censusMoved, "B1, #2016 (local half); PR 10 leftover, #2083 (open) (--server half)", "split across two homes: the local-host half (create config/keys/DB) moved to `admin init`; the --server network-bootstrap half (POST /system/init, unauthenticated, bootstrap-token-gated) moved to the thin CLI's own `system init --server`"},
	...
	"usage show":                          {censusMoved, "FINISH-SPLIT census-gaps", "ported the REST/remote branch only (cli/cmd/usage.go); local/embedded branch dropped, not carried forward -- Finding S17"},
