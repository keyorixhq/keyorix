...
// prefix is "" for the primary passphrase or "new-" for a second one.
//
// A thin re-export of crypto.RegisterPassphraseFlags (moved there per
// docs/cli-split-inventory.md §7 PR 12, so server/admin and
// internal/encryptionops can register the same flags without importing this
// package's much larger CLI-specific dependency surface) — kept here so
// existing internal/cli/encryption call sites are unaffected.
func RegisterPassphraseFlags(fs *pflag.FlagSet, src *crypto.PassphraseSource, prefix, human string) (fdFlag, fileFlag, stdinFlag string) {
	return crypto.RegisterPassphraseFlags(fs, src, prefix, human)
}
