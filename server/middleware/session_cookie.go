...
func ClearSessionCookie(w http.ResponseWriter, secure bool) {
	http.SetCookie(w, &http.Cookie{ // #nosec G124 -- nosemgrep: go.lang.security.audit.net.cookie-missing-secure.cookie-missing-secure -- NOSONAR go:S2092 false positive: Secure flag threaded from runtime TLS config
		Name:     SessionCookieName,
		Value:    "",
		Path:     "/",
		HttpOnly: true,
...
func SetCSRFCookie(w http.ResponseWriter, token string, secure bool) {
	http.SetCookie(w, &http.Cookie{ // #nosec G124 -- nosemgrep: go.lang.security.audit.net.cookie-missing-secure.cookie-missing-secure,go.lang.security.audit.net.cookie-missing-httponly.cookie-missing-httponly -- NOSONAR go:S2092 go:S3330 false positive: HttpOnly:false intentional for CSRF double-submit; Secure from runtime TLS
		Name:     CSRFCookieName,
		Value:    token,
		Path:     "/",
		HttpOnly: false,
