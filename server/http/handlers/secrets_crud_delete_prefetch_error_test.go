// secrets_crud_delete_prefetch_error_test.go — regression coverage for
// DeleteSecret's prefetch-permission-check fix (found live by
// FuzzStorageFaultOperations, corpus entry 52dd4afe85dd34bb): the audit-log
// name prefetch (GetSecretWithPermissionCheck) is also this route's own
// permission-resolution read for the non-machine path, and its error must
// now abort the request instead of being silently discarded.
//
// TestDeleteSecret_NotBlockedByAccessSchedule locks in the one deliberate
// carve-out that fix needed: a SecretAccessSchedule pins a READ window
// (secret_schedule.go's own doc comment) and DeleteSecretWithPermissionCheck
// never enforces it — deletion is intentionally not schedule-gated. Without
// exempting ErrAccessOutsideSchedule specifically, the prefetch-error-aborts
// fix would have newly (and wrongly) imposed that read-only restriction onto
// delete.
package handlers
