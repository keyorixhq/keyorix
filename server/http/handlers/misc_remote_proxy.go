...
func (h *ShareHandler) ListSharesByOwnerProxy(w http.ResponseWriter, r *http.Request) {
	...
	shares, err := h.coreService.Storage().ListSharesByOwner(r.Context(), uint(ownerID), h.coreService.ShareEffectiveNow())
	if err != nil {
...
func (h *ShareHandler) ListSharesByUserProxy(w http.ResponseWriter, r *http.Request) {
	...
	shares, err := h.coreService.Storage().ListSharesByUser(r.Context(), uint(userID), h.coreService.ShareEffectiveNow())
	if err != nil {
