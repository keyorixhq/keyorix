...
var permissionSweepAllowlist = map[string]string{
	"server/http/router.go:394": "GET /notification-channels/{id}/retry-policy returns only " +
	...
	"server/http/router.go:417": "GET /dashboard/stats is the caller's OWN home dashboard. " +
	...
	"server/http/router.go:427": "GET /system/auth-config returns a deliberately redacted, " +
	...
	"server/http/router.go:428": "GET /system/encryption-config returns a deliberately " +
	...
	"server/http/router.go:435": "GET /system/info returns server version/build/runtime info " +
		"(no per-tenant data) -- the same deployment-wide, non-disclosure-sensitive shape as " +
		"auth-config/encryption-config above.",
	"server/http/router.go:436": "GET /system/metrics returns process-level runtime metrics " +
	...
	"server/http/router.go:1080": "GET /audit/anomalies sits inside r.Route(\"/audit\", ...) " +
	...
	"server/http/router.go:2087": "GET /license/status returns deployment-wide license " +
		"metadata (plan/features/seat count/expiry) -- not scoped to any tenant/project/user, " +
		"nothing to cross-tenant-disclose.",
	"server/http/router.go:2183": "GET /sod/policies returns policy DEFINITIONS (name + the " +
	...
	"server/http/router.go:2230": "GET /admin/anomaly-config returns the DB-persisted anomaly " +
...
var noPermissionGateAllowlist = map[string]string{
	...
	"server/http/router.go:265":  justPublicInfra + " GET /status-es is the Spanish-language mirror of /status immediately above.",
	"server/http/router.go:2241": justPublicInfra + " Swagger UI is gated by cfg.Server.HTTP.SwaggerEnabled (a deployment config flag, not a per-caller permission) -- see the adjacent comment; the machine-readable API surface it exposes is the same shape /openapi.yaml exposes below.",
	"server/http/router.go:2242": justPublicInfra + " GET /openapi.yaml is the raw OpenAPI spec, gated by the same cfg.Server.HTTP.SwaggerEnabled flag as the Swagger UI immediately above (#224 fixed the two having diverging on/off behavior; they must stay paired).",

	"server/http/router.go:336": justSelfServiceOwnAccount + " GET/PUT /auth/profile.",
	"server/http/router.go:337": justSelfServiceOwnAccount + " GET/PUT /auth/profile.",
	"server/http/router.go:338": justSelfServiceOwnAccount + " POST /auth/change-password changes only the caller's own password.",
	"server/http/router.go:343": justSelfServiceOwnAccount + " POST /auth/mfa/enroll starts enrolling the caller's OWN second factor; blocked under impersonation (BlockWhenImpersonating) so an admin acting as a user cannot plant a durable credential.",
	"server/http/router.go:344": justSelfServiceOwnAccount + " POST /auth/mfa/activate activates the caller's OWN pending enrolment; same impersonation block as enroll above.",
	"server/http/router.go:345": justSelfServiceOwnAccount + " POST /auth/mfa/disable disables only the caller's OWN MFA; same impersonation block.",
	"server/http/router.go:346": justSelfServiceOwnAccount + " GET /auth/mfa/recovery-codes/status reads only the caller's OWN recovery-code status.",
	"server/http/router.go:347": justSelfServiceOwnAccount + " POST recovery-codes/regenerate regenerates only the caller's OWN codes; same impersonation block.",
	"server/http/router.go:352": justSelfServiceOwnAccount + " POST /auth/mfa/stepup re-verifies the caller's OWN TOTP/recovery code to open their own restricted-secret read window; same impersonation block.",
	"server/http/router.go:354": justSelfServiceOwnAccount + " POST webauthn/register/begin registers a passkey for the caller's OWN account; same impersonation block.",
	"server/http/router.go:355": justSelfServiceOwnAccount + " POST webauthn/register/finish completes the same ceremony as register/begin above.",
	"server/http/router.go:356": justSelfServiceOwnAccount + " GET webauthn/credentials lists only the caller's OWN passkeys.",
	"server/http/router.go:361": justSelfServiceOwnAccount + " DELETE webauthn/credentials/{id} deletes only the caller's OWN passkey (there is no admin API to remove another user's passkey); same impersonation block, since deleting the last passkey is the same durable MFA-downgrade /auth/mfa/disable is blocked from doing.",
	"server/http/router.go:367": justSelfServiceOwnAccount + " POST /auth/webauthn/reauth/begin starts a live passkey re-assertion for the caller's OWN account (the WebAuthn-only path to satisfy requireReauth); same impersonation block as the other WebAuthn/MFA self-service routes.",
	"server/http/router.go:368": justSelfServiceOwnAccount + " POST /auth/webauthn/reauth/finish completes the same ceremony as reauth/begin immediately above.",
	"server/http/router.go:369": justSelfServiceOwnAccount + " GET /auth/sessions lists only the caller's OWN sessions.",
	"server/http/router.go:370": justSelfServiceOwnAccount + " DELETE /auth/sessions/{id} revokes only one of the caller's OWN sessions.",
	"server/http/router.go:371": justSelfServiceOwnAccount + " GET /auth/tokens lists only the caller's OWN PATs.",
	"server/http/router.go:374": justSelfServiceOwnAccount + " POST /auth/tokens mints a PAT for the caller's OWN account; blocked under impersonation so an admin acting as a user cannot plant a durable token.",
	"server/http/router.go:375": justSelfServiceOwnAccount + " DELETE /auth/tokens/{id} revokes only one of the caller's OWN PATs.",
	"server/http/router.go:377": justSelfServiceOwnAccount + " GET tokens/expired lists only the caller's OWN expired PATs.",
	"server/http/router.go:378": justSelfServiceOwnAccount + " DELETE tokens/expired bulk-revokes only the caller's OWN expired PATs.",
	"server/http/router.go:380": justSelfServiceOwnAccount + " POST /auth/end-impersonation ends only the CALLER'S OWN impersonation session.",
	"server/http/router.go:383": justSelfServiceOwnAccount + " GET /notifications lists only the caller's OWN in-app notifications (ADR-024).",
	"server/http/router.go:384": justSelfServiceOwnAccount + " POST notifications/read-all marks only the caller's OWN notifications read.",
	"server/http/router.go:385": justSelfServiceOwnAccount + " POST notifications/{id}/read marks one of the caller's OWN notifications read.",

	"server/http/router.go:512": "Self-service (ADR-024): POST /projects/{id}/access-requests lets an " +
	...
	"server/http/router.go:514": "Self-service (ADR-024): POST access-requests/{requestId}/withdraw withdraws " +
	...
	"server/http/router.go:530": "Self-service by design: POST /projects/{id}/break-glass activates " +
	...
	"server/http/router.go:628": "GET /secrets (ListSecrets) performs its own authorization INSIDE the " +
	...
	"server/http/router.go:630": "GET /secrets/policy returns the deployment's ACTIVE create-time naming/" +
	...
	"server/http/router.go:724": "POST /secrets (CreateSecret) is authorized INSIDE the handler: scope " +
	...
	"server/http/router.go:745": "DELETE /secrets/{id}/self-share (RemoveSelfFromShare) removes only the " +
		...
		"adjacent comment.",
	"server/http/router.go:770": "POST /folders (CreateFolder) is authorized INSIDE the handler: scope " +
		...
		"body)\").",
	"server/http/router.go:792": "POST /rotation-policies (Create) is authorized INSIDE the handler: scope " +
	...
	"server/http/router.go:821": "POST /dynamic-secrets/configs (CreateConfig) is authorized INSIDE the " +
	...
	"server/http/router.go:822": "GET /dynamic-secrets/configs (ListConfigs) is authorized INSIDE the " +
