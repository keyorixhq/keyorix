...
var classifiedNodeCredentialRoutes = []nodeRouteEntry{
	...
	{"POST", "/api/v1/system/machine-identities/{id}/roles/{roleId}", classNodeLegitimate,
		"AssignMachineRoleProxy: #1542 FIXED -- routes through core.AssignMachineRole unconditionally, restoring machineInProject scope-bounding + requireAuthorityForRole"},
	...
	{"POST", "/api/v1/system/rbac/remove-all-project-role-grants", classNodeLegitimate,
		"RemoveAllProjectRoleGrantsProxy: #1542 FIXED -- routes through core.RemoveProjectMember unconditionally, restoring guardLastProjectAdmin (target-state, like row 1); core.ErrNotProjectMember mapped back to success for idempotency parity with the old raw delete"},
